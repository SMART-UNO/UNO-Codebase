...
class DQNAgent:
    def __init__(self, num_actions, state_size=240, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"), gamma=0.99, learning_rate=0.001, batch_size=64, memory_size=10000, epsilon_start=1.0, epsilon_end=0.01, epsilon_decay=0.995, update_frequency=1000):
        ...
        self.q_network = QNetwork(
            num_actions=num_actions, state_size=state_size, device=device).to(device)
    ...
    def step(self, state):
        ...
        if np.random.rand() > self.epsilon:
            ...
            action = max(legal_actions.keys(),
                         key=lambda x: action_values.cpu().data.numpy()[0][x])
        else:
    ...
    def update(self):
        ...
        states = np.vstack(states_obs).reshape(
        ...
        next_states = np.vstack(next_states_obs).reshape(
            self.batch_size, -1).astype(np.float32)
